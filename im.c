...
// Parser state
static key_sequence_event_t *toggle_sequence = NULL;
static size_t toggle_sequence_len = 0;
static key_sequence_event_t *event_history = NULL;
...
static void parse_toggle_sequence(void) {
        size_t max_events = toggle_sequence_str_len * 10;
        toggle_sequence = calloc(max_events, sizeof(key_sequence_event_t));
        if (!toggle_sequence) {
                wlpinyin_err("Failed to allocate toggle sequence");
                return;
        }

        size_t idx = 0;
        for (size_t seq_idx = 0; seq_idx < toggle_sequence_str_len; seq_idx++) {
                char *str = strdup(toggle_sequence_str[seq_idx]);
                if (!str) continue;
                ...
                // All are hold combos: modifiers + main key
                unsigned int modmask = 0;

                // All but last are modifiers
                for (int tok_idx = 0; tok_idx < token_count - 1; tok_idx++) {
                        xkb_keysym_t mod = parse_keysym(tokens[tok_idx]);
                        if (mod == XKB_KEY_NoSymbol) {
                                wlpinyin_err("Unknown key: %s", tokens[tok_idx]);
                                continue;
                        }

                        toggle_sequence[idx++] = (key_sequence_event_t){
                                .keysym = mod, .pressed = true, .modmask = modmask
                        };

                        if (mod == XKB_KEY_Shift_L || mod == XKB_KEY_Shift_R)
                                modmask |= ShiftMask;
                        else if (mod == XKB_KEY_Control_L || mod == XKB_KEY_Control_R)
                                modmask |= ControlMask;
                        else if (mod == XKB_KEY_Alt_L || mod == XKB_KEY_Alt_R)
                                modmask |= Mod1Mask;
                        else if (mod == XKB_KEY_Super_L || mod == XKB_KEY_Super_R)
                                modmask |= Mod4Mask;
                }

                xkb_keysym_t main_key = parse_keysym(tokens[token_count - 1]);
                if (main_key == XKB_KEY_NoSymbol) {
                        wlpinyin_err("Unknown key: %s", tokens[token_count - 1]);
                        free(str);
                        continue;
                }

                toggle_sequence[idx++] = (key_sequence_event_t){
                        .keysym = main_key, .pressed = true, .modmask = modmask
                };

                free(str);
        }

        toggle_sequence_len = idx;

        event_history = calloc(toggle_sequence_len, sizeof(key_sequence_event_t));
        if (!event_history) {
                wlpinyin_err("Failed to allocate event history");
        }

        wlpinyin_dbg("Parsed %zu toggle sequence events", toggle_sequence_len);
}

static bool check_modifiers(struct xkb_state *xkb, unsigned int expected_mask) {
        if (expected_mask == 0) return true;

        xkb_mod_mask_t active = xkb_state_serialize_mods(xkb, 
                                                         XKB_STATE_MODS_EFFECTIVE | XKB_STATE_LAYOUT_EFFECTIVE);

        struct xkb_keymap *keymap = xkb_state_get_keymap(xkb);
        unsigned int active_mask = 0;

        xkb_mod_index_t shift_idx = xkb_keymap_mod_get_index(keymap, XKB_MOD_NAME_SHIFT);
        xkb_mod_index_t ctrl_idx = xkb_keymap_mod_get_index(keymap, XKB_MOD_NAME_CTRL);
        xkb_mod_index_t alt_idx = xkb_keymap_mod_get_index(keymap, XKB_MOD_NAME_ALT);
        xkb_mod_index_t logo_idx = xkb_keymap_mod_get_index(keymap, XKB_MOD_NAME_LOGO);

        if (shift_idx != XKB_MOD_INVALID && (active & (1 << shift_idx)))
                active_mask |= ShiftMask;
        if (ctrl_idx != XKB_MOD_INVALID && (active & (1 << ctrl_idx)))
                active_mask |= ControlMask;
        if (alt_idx != XKB_MOD_INVALID && (active & (1 << alt_idx)))
                active_mask |= Mod1Mask;
        if (logo_idx != XKB_MOD_INVALID && (active & (1 << logo_idx)))
                active_mask |= Mod4Mask;

        return (active_mask & expected_mask) == expected_mask;
}

bool im_toggle(struct xkb_state *xkb, xkb_keysym_t keysym, bool pressed) {
        if (!event_history || toggle_sequence_len == 0) {
                return false;
        }

        // Only track press events (hold combos only)
        if (!pressed) 
                return false;

        if (history_count < toggle_sequence_len) {
                event_history[history_count++] = (key_sequence_event_t){
                        .keysym = keysym, .pressed = true, .modmask = 0
                };
        } else {
                memmove(event_history, event_history + 1,
                        (toggle_sequence_len - 1) * sizeof(key_sequence_event_t));
                event_history[toggle_sequence_len - 1] = (key_sequence_event_t){
                        .keysym = keysym, .pressed = true, .modmask = 0
                };
        }
        ...
        for (size_t seq_idx = 0; seq_idx < toggle_sequence_len; seq_idx++) {
                const key_sequence_event_t *expected = &toggle_sequence[seq_idx];
                const key_sequence_event_t *actual = &event_history[seq_idx];

                // Check keysym
                if (actual->keysym != expected->keysym) {
                        return false;
                }

                // Check modifiers if specified
                if (expected->modmask != 0 && !check_modifiers(xkb, expected->modmask)) {
                        return false;
                }
        }
...
int im_destroy(struct wlpinyin_state *state) {
        ...
        if (toggle_sequence != NULL) {
                free(toggle_sequence);
                toggle_sequence = NULL;
        }
